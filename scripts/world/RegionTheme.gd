...
static func apply_shape_modifier(grid: Array, theme: int, rng: RandomNumberGenerator, w: int, h: int) -> void:
	var fc := _RoomAnchorHelper.get_room_floor_ceiling(grid)
	if fc.floor_hi < 0:
		fc.floor_hi = h - 4
		fc.floor_lo = 2
	match theme:
		HexTheme.GEOCRASH: _mod_geocrash(grid, rng, w, h, fc)
		HexTheme.VOIDREND: _mod_voidrend(grid, rng, w, h, fc)
		HexTheme.ECHOSCREAM: _mod_echoscream(grid, rng, w, h, fc)
		HexTheme.MEMOREAVE: _mod_memoreave(grid, rng, w, h, fc)
		HexTheme.NULLPULSE: _mod_nullpulse(grid, rng, w, h, fc)
		HexTheme.TECHNOMANTIC: _mod_technomantic(grid, rng, w, h, fc)
...
static func _mod_geocrash(grid: Array, rng: RandomNumberGenerator, w: int, h: int, fc: Dictionary) -> void:
	var y_limit := mini(h - 2, fc.floor_hi + 2)
	for y in range(2, y_limit):
		for x in [0, w - 1]:
...
static func _mod_voidrend(grid: Array, rng: RandomNumberGenerator, w: int, h: int, fc: Dictionary) -> void:
	var platform_ymax := maxi(3, mini(h - 6, fc.floor_hi - 4))
	for _i in range(rng.randi_range(1, 3)):
		var dx := rng.randi_range(3, w - 4)
		var dy := rng.randi_range(3, platform_ymax)
		if _tile_at(grid, dx, dy) == TILE_EMPTY:
			grid[dy][dx] = TILE_PLATFORM
			if rng.randf() < 0.4:
				grid[dy + 1][dx] = TILE_PLATFORM
	var wall_ymax := maxi(3, mini(h - 4, fc.floor_hi - 2))
	for _i in range(rng.randi_range(1, 4)):
		var tx := rng.randi_range(0, 2) if rng.randf() < 0.5 else w - 1 - rng.randi_range(0, 2)
		var ty := rng.randi_range(3, wall_ymax)
		if _tile_at(grid, tx, ty) == TILE_WALL:
			grid[ty][tx] = TILE_EMPTY

static func _mod_echoscream(grid: Array, rng: RandomNumberGenerator, w: int, h: int, fc: Dictionary) -> void:
	var y_limit := mini(h - 2, fc.floor_hi + 2)
	for y in range(2, y_limit):
		for x in [0, w - 1]:
	...
	var strip_ymax := maxi(3, mini(h - 3, fc.floor_hi + 1))
	for y in range(3, strip_ymax):
		if rng.randf() < 0.1:
...
static func _mod_memoreave(grid: Array, rng: RandomNumberGenerator, w: int, h: int, fc: Dictionary) -> void:
	var floor_ymax := maxi(2, mini(h - 3, fc.floor_hi + 1))
	for _i in range(rng.randi_range(1, 3)):
		var gx := rng.randi_range(2, w - 3)
		var gy := rng.randi_range(2, floor_ymax)
	...
	var plat_ymax := maxi(3, mini(h - 5, fc.floor_hi - 1))
	for _i in range(rng.randi_range(1, 2)):
		var ox := rng.randi_range(3, w - 5)
		var oy := rng.randi_range(3, plat_ymax)
...
static func _mod_nullpulse(grid: Array, rng: RandomNumberGenerator, w: int, h: int, fc: Dictionary) -> void:
	var cx := w / 2
	var cy := maxi(4, mini(h - 5, fc.floor_hi - 3))
	for _i in range(rng.randi_range(1, 2)):
...
static func _mod_technomantic(grid: Array, rng: RandomNumberGenerator, w: int, h: int, fc: Dictionary) -> void:
	var y_limit := maxi(3, mini(h - 3, fc.floor_hi + 1))
	for y in range(3, y_limit):
		for x in [2, w - 3]:
			if _tile_at(grid, x, y) == TILE_WALL and rng.randf() < 0.15:
				var dir := 1 if x < w / 2 else -1
				grid[y][x + dir] = TILE_PLATFORM
	var plat_ymax := maxi(4, mini(h - 5, fc.floor_hi - 1))
	for _i in range(rng.randi_range(1, 3)):
		var gx := rng.randi_range(4, w - 5)
		var gy := rng.randi_range(4, plat_ymax)
